...
class Object:
    def __init__(self, id, x, y, width, height, bounce, point, c):
        self.id = id
        self.x = x
        self.y = y
        self.width = width
    ...
    def collision(self, ball, obj, plunger=False):
        ...
        # objの当たり判定
        if (self.x <= ball.x <= self.x + self.width or self.x <= ball.x + ball.d <= self.x + self.width) and (self.y <= ball.y <= self.y + self.height or self.y <= ball.y + ball.d <= self.y + self.height):
            if (self.x < ball.x or self.x < ball.x + ball.d) and (self.x + self.width > ball.x + ball.d or self.x + self.width > ball.x):
                # obj上壁の当たり判定
...
class Paddle:
    ...
    def collision(self, ball, obj):
        ...
        if ((ball.x + ball.d >= self.x) and (ball.x < self.x + self.width)) and (self.y + self.height > ball.y + ball.d >= self.y):
...
while not exiting:
    ...
    for wall in walls_class:
        paddle.collision(ball, wall)
    for obj in objects_class:
        obj.collision(ball, obj)
    plunger.collision(ball, plunger, True)
