...
class Box:
    ...
    def check_paddle(self, ball):
        global timing
        if ((ball.x + ball.d >= self.paddle.x) and (ball.x < self.paddle.x + self.paddle.w)) and (self.paddle.y + self.paddle.h > ball.y + ball.d >= self.paddle.y) and timing > 2:
            timing = 0
            ball.vy = -ball.vy
            ball.timer = 20
            ball.falling = False
    ...
    def animate(self):
        ...
        for x in range(1000):
            for ball in self.balls:
                ...
                ball.redraw()
            timing += 1
            self.paddle.moveP(self.lefting, self.righting)
            self.paddle.redraw()
