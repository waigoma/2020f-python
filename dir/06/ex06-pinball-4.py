...
class Paddle:
    ...
    w: int
    h: int
    dx: int = field(init=False, default=10)

    def move(self, lefting, righting):  # パドルを動かす
        if lefting:
            self.x -= self.dx
        if righting:
            self.x += self.dx
    ...
    def collision(self, __box, ball):
        global timing
        if ((ball.x + ball.d >= self.x) and (ball.x < self.x + self.w)) and (
                self.y + self.h > ball.y + ball.d >= self.y) and timing > 2:
            timing = 0
            ball.vy = -ball.vy
            ball.timer = 20
            ball.falling = False
    ...
    def animate(self):
        ...
        for x in range(1000):
            for ball in self.balls:
                ...
                ball.redraw()
            timing += 1
            self.paddle.move(self.lefting, self.righting)
            self.paddle.redraw()
